...
DEFAULT_REPO_URL="http://mirror.cyberbits.eu/archlinux"
DEFAULT_ARM_REPO_URL="http://mirror.archlinuxarm.org"

# retroroot
if [ "${RR_PLATFORM}" == "desktop" ]; then
  EXTRA_PACKAGES="${EXTRA_PACKAGES} linux linux-firmware"
elif [ "${RR_PLATFORM}" == "rpi" ]; then
  echo "TODO"
  exit 1
fi
...
configure_minimal_system() {
  ...
  echo "retroroot" > "$DEST/etc/hostname"
...
main() {
  ...
  # retroroot: mount bindings for linux (kernel) installation
